...
- **35%** of MTN Nigeria OPEX is energy-related — the single largest controllable cost line *(ALTON, 2025)*
- **₦1,758/litre** current diesel price — 25% higher than last year, and still rising *(NBS, May 2025)*
- **₦120–140 billion** estimated annual profit at risk from diesel costs alone *(TechCabal, 2026)*
- **$5.5 million/month** MTN spends on diesel across its generator fleet *(IEEE Spectrum)*
- **70%** of all base station downtime is caused by power failures, not network faults *(NCC/industry)*
- **1,344** diesel theft incidents in 2025 — average ₦3.5 million per stolen generator *(Nairametrics, 2026)*
- **94%** anomaly detection confidence for fuel theft events — from `AnomalyEvent.Detect()` in codebase
- **Up to 60%** diesel cost offset achievable with solar-hybrid deployment — 3–6 year payback *(Royal Power, 2025)*
...
- **₦0.12 million daily OPEX reduction** per 1% increase in solar adoption across the demo fleet — from codebase optimization coefficients
- **2–4 weeks earlier** generator fault prediction vs reactive maintenance — from `PredictedFault` confidence scoring **[TO VALIDATE — add simulation data before demo]**
- **Estimated ₦50–80 million/year OPEX savings** per 100-site cluster through AI-optimized dispatch and source switching **[TO VALIDATE — run optimization simulation across seeded fleet]**
...
| Threshold-based SCADA alerting | Confidence-scored ML anomalies: not "diesel < 20%" but "this consumption curve matches fuel theft at 94% probability — act now" |
...
- **Fuel theft is a uniquely Nigerian crisis:** 1,344 theft incidents in 2025, 656 stolen assets — a rate that has no parallel in European or East African markets. TelcoPilot's `FuelTheft` anomaly detection is not a generic feature; it is built because Lagos demanded it.
...
- **Data validation via FluentValidation** — `ValidationPipelineBehavior` rejects malformed commands before they touch the database — NOT FOUND in codebase as explicit validators yet: **[add FluentValidation rules to Energy commands before demo day]**
...
**[Slide 7]** | From 15 sites to 20,000 — one Docker command | Three-panel flow: `docker compose up` → fleet online → scale slider showing site count → ₦/day OPEX curve dropping | *Deployment realism is the judges' biggest doubt — kill it on this slide.*
...
| Integrations | OpenStreetMap (geo-enrichment via `OsmClient`), MCP plugin architecture (extensible AI tool use), Azure OpenAI / OpenAI-compatible API |
...
- [ ] **Real IoT data ingestion** — `EnergyTickerService` simulates telemetry; show a clear slide explaining simulation vs. production integration path
- [ ] **SMS/WhatsApp alert dispatch** — `INotificationService` interface exists but `NotificationService.cs` implementation is a stub; wire up a single Twilio or Africa's Talking call before demo day

### Numbers and metrics needing simulation data
- [ ] **Fleet-wide OPEX savings projection** — run `GetOptimizationProjectionQuery` across the full 15-site seeded fleet at 40% solar and export the ₦ figure for the slide deck
- [ ] **Outage prevention window** — calculate: `PredictedFault` anomaly lead time (hours before actual failure) from `EnergyPrediction` records in seed data; use as "X-hour early warning" stat
- [ ] **Fuel theft ROI** — calculate ₦ value of diesel protected by FuelTheft detection across 2025 simulated data; compare to ₦3.5M/generator replacement cost
- [ ] **CO₂ impact** — run optimization model at 20%, 40%, 60% solar to produce a CO₂ avoidance table for the sustainability slide

### Enterprise signals not yet fully implemented
- [ ] **FluentValidation rules** — `ValidationPipelineBehavior` is wired but Energy module commands lack explicit validators; add before demo to demonstrate input hardening
- [ ] **Notification Service** — `INotificationService` is an empty stub in `NotificationService.cs`; add at minimum a console/webhook implementation so the demo can show "alert dispatched"
- [ ] **Rate limiting** — no rate limiting middleware on the API; add `dotnet add package Microsoft.AspNetCore.RateLimiting` and configure before demo

### Nigeria-specific context not yet reflected in the UI
- [ ] **Diesel price displayed as ₦1,758/litre** — codebase optimization model uses ₦700/litre baseline (pre-subsidy-removal price); update the default `dieselPriceNgnPerLitre` in the optimize page to ₦1,758 to match 2025 market reality — this dramatically increases the ROI numbers
- [ ] **NCC compliance callout** — add a dashboard banner referencing the April 2026 automatic compensation mandate; makes the business case concrete for judges
- [ ] **MTN NOC document titles visible in copilot responses** — confirm `MTN_NOC_Lagos_SOP.pdf` is indexed in the seeded RAG corpus and that copilot responses cite it by name; this is the single biggest credibility signal for MTN judges
