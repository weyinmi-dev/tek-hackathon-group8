...
const ROLE_CAPS: Record<string, string[]> = {
  ...
  manager:  ["copilot.read", "copilot.write", "alerts.read", "alerts.assign", "users.create", "users.update", "reports.export", "map.read", "dashboard.read"],
...
function UsersInner() {
  ...
  const refresh = async () => {
    try {
      ...
      setUsers(r);
      setSel(prev => prev ? r.find(u => u.id === prev.id) ?? r[0] ?? null : r[0] ?? null);
      setErr(null);
    } catch (e) {
      setErr(String(e));
    }
  ...
  const canEditSel = !!sel && !!me && canManageTarget(me.role, sel.role) && sel.id !== me.id;
  const canDeleteSel = !!sel && !!me && isAdmin(me.role) && sel.id !== me.id;

  const onChangeRole = async (role: string) => {
    if (!sel) return;
    setBusy(true);
    try { await api.changeUserRole(sel.id, role); await refresh(); }
  ...
  const onToggleActive = async () => {
    if (!sel) return;
    setBusy(true);
    try { await api.setUserActive(sel.id, !sel.isActive); await refresh(); }
    catch (e) { setErr(String(e)); }
    finally { setBusy(false); }
  };
  ...
  const onDelete = async () => {
    if (!sel) return;
    if (!window.confirm(`Delete ${sel.fullName}? This cannot be undone.`)) return;
    setBusy(true);
    try { await api.deleteUser(sel.id); setSel(null); await refresh(); }
    catch (e) { setErr(String(e)); }
    finally { setBusy(false); }
  };
  ...
  return (
    <>
      <TopBar
        ...
        right={isManager(me?.role) ? <Btn primary onClick={() => setCreateOpen(true)}>+ Invite user</Btn> : undefined}
      />
      <div style={{ padding: 22, display: "grid", gridTemplateColumns: "1fr 380px", gap: 14 }}>
        ...
        <Card pad={0}>
          <div style={{
            ...
            <span>USER</span><span>ROLE</span><span>STATE</span><span>TEAM</span><span>REGION</span><span>LAST ACTIVE</span>
          </div>
          {users.map((u, i) => {
            ...
            return (
              <button key={u.id} onClick={() => setSel(u)} style={{
                ...
                <Pill tone={u.isActive ? "ok" : "neutral"} dot>{u.isActive ? "active" : "inactive"}</Pill>
          ...
          {sel && (
            <>
              <Card pad={16}>
                ...
                <Row k="Created" v={new Date(sel.createdAtUtc).toLocaleString()} />
                ...
                <Section label="ACTIONS">
                  <Card pad={14} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      <Btn small onClick={() => setEditOpen(true)} disabled={busy}>Edit profile</Btn>
                      <Btn small onClick={onToggleActive} disabled={busy}>
                        {sel.isActive ? "Deactivate" : "Activate"}
                      </Btn>
                      {canDeleteSel && <Btn small onClick={onDelete} disabled={busy} style={{ color: "var(--crit)" }}>Delete</Btn>}
                    </div>
                    <div>
                      <div className="mono uppr" style={{ fontSize: 9, color: "var(--ink-3)", letterSpacing: ".12em", marginBottom: 6 }}>Change role</div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                        {ROLES.map(r => {
                          const disabled = busy || (isManager(me?.role) && !isAdmin(me?.role) && r === "admin");
                          return (
                            <Btn key={r} small disabled={disabled || sel.role === r}
...
function CreateUserModal({ actorRole, onClose, onCreated }: { actorRole?: string; onClose: () => void; onCreated: () => void }) {
  const [form, setForm] = useState({
    email: "", password: "", fullName: "", handle: "",
    role: "engineer", team: "", region: "All regions",
  });
  const [err, setErr] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  ...
  return (
    <Modal title="Create user" onClose={onClose}>
      <form onSubmit={submit} style={{ display: "grid", gap: 10 }}>
        <Field label="Full name"><input style={inputStyle} required value={form.fullName} onChange={e => setForm({ ...form, fullName: e.target.value })} /></Field>
        <Field label="Handle"><input style={inputStyle} required value={form.handle} onChange={e => setForm({ ...form, handle: e.target.value })} /></Field>
        ...
        <Field label="Temp password (≥8 chars)"><input style={inputStyle} type="password" required minLength={8} value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} /></Field>
        <Field label="Role">
          <select style={inputStyle} value={form.role} onChange={e => setForm({ ...form, role: e.target.value })}>
            {allowedRoles.map(r => <option key={r} value={r}>{r}</option>)}
          </select>
        </Field>
        <Field label="Team"><input style={inputStyle} value={form.team} onChange={e => setForm({ ...form, team: e.target.value })} /></Field>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 6 }}>
          ...
          <Btn type="submit" primary disabled={busy}>{busy ? "Creating…" : "Create"}</Btn>
        </div>
      </form>
    </Modal>
  );
...
function EditUserModal({ user, onClose, onSaved }: { user: UserListItem; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ fullName: user.fullName, handle: user.handle, team: user.team, region: user.region });
  const [err, setErr] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setBusy(true); setErr(null);
    try { await api.updateUser(user.id, form); onSaved(); }
    catch (e) { setErr(String(e)); }
    finally { setBusy(false); }
  };
  return (
    <Modal title={`Edit ${user.handle}`} onClose={onClose}>
      <form onSubmit={submit} style={{ display: "grid", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 6 }}>
          <Btn type="button" onClick={onClose}>Cancel</Btn>
          <Btn type="submit" primary disabled={busy}>{busy ? "Saving…" : "Save"}</Btn>
        </div>
      </form>
    </Modal>
  );
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        width: 420, padding: 20, boxShadow: "0 20px 60px rgba(0,0,0,.45)",
      }}>
