...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    api.map().then(r => {
      if (!alive) return;
      setData(r);
      setSel(r.towers.find(t => t.status === "critical") ?? r.towers[0] ?? null);
    });
    return () => { alive = false; };
  }, []);
  ...
  return (
    <>
      ...
      <div style={{ padding: 22, display: "grid", gridTemplateColumns: "1fr 320px", gap: 14, height: "calc(100vh - 67px)" }}>
        <div>
          {data && <NetworkMap towers={data.towers} onSelect={setSel} selectedId={sel?.id} />}
        </div>
          ...
          {sel && (
            <Card pad={14}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                ...
                <Pill tone={sel.status === "critical" ? "crit" : sel.status === "warn" ? "warn" : "ok"} dot>{sel.status}</Pill>
              </div>
              {sel.issue && (
                <div style={{
          ...
          <Section label="REGIONS">
            <Card pad={0}>
              {(data?.regions ?? []).map((r, i, all) => {
                const tone = r.critical > 0 ? "crit" : r.warn > 0 ? "warn" : "ok";
                return (
                  <div key={r.name} style={{
                    ...
                    <div>
                      ...
                      <div className="mono" style={{ fontSize: 10, color: "var(--ink-3)", marginTop: 2 }}>{r.towers} towers</div>
                    </div>
                    <Pill tone={tone} dot>
                      {r.critical ? `${r.critical} crit` : r.warn ? `${r.warn} warn` : "ok"}
                    </Pill>
                  </div>
                );
              })}
...
function Metric({ label, v, unit, tone }: { label: string; v: number; unit: string; tone: "ok" | "warn" | "crit" }) {
  return (
    <div>
      ...
      }}>{v}<span style={{ color: "var(--ink-3)", fontSize: 11, marginLeft: 2 }}>{unit}</span></div>
      <Bar pct={v} tone={tone} />
    </div>
  );
}
