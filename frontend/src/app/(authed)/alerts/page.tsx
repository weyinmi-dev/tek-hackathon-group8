...
export default function AlertsPage() {
  ...
  async function load() {
    const r = await api.alerts({ severity: filter === "all" ? undefined : filter });
    setAlerts(r);
    setSel(prev => prev ? r.find(a => a.id === prev.id) ?? r[0] ?? null : r[0] ?? null);
  }
          ...
          <div style={{ display: "flex", gap: 6, padding: 3, background: "var(--bg-1)", border: "1px solid var(--line)", borderRadius: 7 }}>
            {(["all", "critical", "warn", "info"] as const).map(k => (
              <button key={k} onClick={() => setFilter(k)} style={{
                ...
                <span className="mono" style={{ fontSize: 9.5, color: "var(--ink-3)" }}>{counts[k]}</span>
              </button>
            ))}
          </div>
        }
            ...
            <button key={a.id} onClick={() => setSel(a)} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <Pill tone={a.sev === "critical" ? "crit" : a.sev === "warn" ? "warn" : "info"} dot>{a.sev}</Pill>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, overflowY: "auto" }}>
            <Card pad={16}>
              <div style={{ marginBottom: 10 }}>
                <Pill tone={sel.sev === "critical" ? "crit" : sel.sev === "warn" ? "warn" : "info"} dot>{sel.id}</Pill>
              </div>
            ...
            {user?.role !== "viewer" && (
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                <Btn primary onClick={() => ack(sel.id)} disabled={acking === sel.id || sel.status === "acknowledged"}>
                  {acking === sel.id ? "Acknowledging…" : sel.status === "acknowledged" ? "Acknowledged" : "Acknowledge"}
                </Btn>
                <Btn>Assign</Btn>
                <Btn>Dispatch field</Btn>
              </div>
            )}
          </div>
        )}
