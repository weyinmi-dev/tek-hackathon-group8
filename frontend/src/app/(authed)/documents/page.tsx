...
export default function DocumentsPage() {
  ...
  const refresh = async () => {
    try {
      const [d, p] = await Promise.all([
        api.documents(page, 10, debouncedSearch),
        api.documentProviders(),
      ]);
      setDocs(d.items);
  ...
  useEffect(() => {
    const inFlight = docs.some(
      (d) => d.status === "Pending" || d.status === "InProgress",
    );
    if (!inFlight) return;
  ...
  const totalSize = useMemo(
    () => docs.reduce((s, d) => s + d.sizeBytes, 0),
    [docs],
  );
        ...
        right={
          <div style={{ display: "flex", gap: 6 }}>
            {isEngineer(user?.role) && (
              <>
                <Btn primary onClick={() => setIngestOpen(true)}>
                  + Ingest log
                </Btn>
                <Btn
            ...
            {isManager(user?.role) && (
              <>
                ...
                <Btn primary onClick={() => setUploadOpen(true)}>
                  + Upload (AI Verified)
                </Btn>
              </>
            )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {runs.length > 0 && (
            <Section label="RECENT PIPELINE RUNS (THIS SESSION)">
              ...
              >
                {runs.map((r) => (
                  <RunSummaryCard key={r.localId} run={r} />
                ))}
              </div>
            </Section>
          )}
            ...
            <Card pad={0}>
              ...
              <div
                style={{
                  ...
                  borderBottom: "1px solid var(--line)",
                  display: "grid",
                  gridTemplateColumns: "2.4fr .9fr 1fr 1fr 1fr 1fr 100px",
                  gap: 10,
                  fontSize: 10,
                  fontFamily: "var(--mono)",
                  color: "var(--ink-3)",
              ...
              {docs.length === 0 && (
                ...
                >
                  No documents yet.{" "}
                  {isManager(user?.role)
                    ? "Upload a runbook or link a cloud-stored SOP to get started."
              ...
              {docs.map((d, i) => (
                <div
                  ...
                  style={{
                    padding: "12px 14px",
                    borderBottom:
                      i < docs.length - 1 ? "1px solid var(--line)" : 0,
                    display: "grid",
                    gridTemplateColumns: "2.4fr .9fr 1fr 1fr 1fr 1fr 100px",
                    gap: 10,
                    alignItems: "center",
              ...
              {total > 10 && (
                ...
                >
                  ...
                  >
                    ...
                    {Math.min(page * 10, total)} of {total} documents
                  </span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <Section label="STORAGE PROVIDERS">
            <Card pad={14}>
              ...
              >
                Cloud providers list as &ldquo;placeholder&rdquo; until an SDK
                adapter is wired in Modules.Ai.Infrastructure →
                DocumentStorageRegistry. The OneDrive log-fetch flow below works
                today via direct-download share links.
              </div>
            </Card>
          </Section>
          ...
          <Section label="AI PIPELINE · NETWORK LOGS">
            <Card pad={14}>
              ...
              >
                Log files (.csv / .json / .jsonl / .xlsx / .txt / .log) trigger
              ...
              <Step
                n="4"
                label="Persist"
                sub="Stage-4 dedup (create vs update)"
              />
              <Step
                n="5"
                label="Project"
                sub="Fan out to dashboards & copilot"
                last
              />
              <div
                style={{
              ...
              <Trigger
                ...
                detail="created or de-duplicated"
              />
              <Trigger label="Optimizations" detail="per-tower proposals" />
              <Trigger
                label="Dashboard insights"
                detail="ingestion analytics row"
              />
              <Trigger
                label="Copilot knowledge base"
                detail="event bus → KB indexer"
                last
              />
            </Card>
          </Section>
          ...
          <Section label="RAG PIPELINE · DOCUMENTS">
            <Card pad={14}>
              <Step
                n="1"
                label="Source"
                sub="Local upload / Google Drive / OneDrive / SharePoint / Azure Blob"
              />
              <Step
                n="2"
                label="Ingestion"
                sub="Stream bytes from the storage provider"
              />
              <Step
                n="3"
                label="Extract"
                sub="text/markdown today; PDF/Office adapter is pluggable"
              />
              <Step
                n="4"
                label="Chunk"
                sub="Recursive splitter (600 chars, 80 overlap)"
              />
              <Step
                n="5"
                label="Embed"
                sub="Azure OpenAI text-embedding-3-small (or hashing fallback)"
              />
              <Step
                n="6"
                label="pgvector"
                sub="Indexed chunks ready for retrieval"
                last
              />
            </Card>
          </Section>
      ...
      >
        <div style={{ minWidth: 0 }}>
          ...
          >
            ...
            <Pill tone="neutral">
              {run.source === "onedrive" ? "OneDrive" : "Local upload"}
            </Pill>
          </div>
          <div
            className="mono"
          ...
          >
            run {summary.ingestionRunId.slice(0, 8)}… · sha{" "}
            {summary.contentHash.slice(0, 10)}… · status {summary.finalStatus}
          </div>
          {summary.failureReason && (
            <div
              className="mono"
              style={{ fontSize: 10, color: "var(--crit)", marginTop: 4 }}
            >
              ⚠ {summary.failureReason}
            </div>
          )}
      ...
      {!failed && (
        ...
        >
          ...
          <Stat
            ...
            value={summary.alertsCreated}
            href="/alerts"
          />
          <Stat
            label="Alerts updated"
            value={summary.alertsUpdated}
            href="/alerts"
          />
          <Stat
            label="Optimizations"
            value={summary.optimizationsCreated}
            href="/optimize"
          />
        </div>
      )}
            ...
            {summary.stageTimings.map((t, i) => {
              ...
              return (
                <Fragment key={i}>
                  <span
                    style={{
                      color: t.succeeded ? "var(--ink-2)" : "var(--crit)",
                    }}
                  >
                    {stageLabel(t.stage)}
                  </span>
                  ...
                  <span
                    style={{ color: t.succeeded ? "var(--ok)" : "var(--crit)" }}
                  >
                    {t.succeeded ? "ok" : "fail"}
                  </span>
                </Fragment>
...
function IngestLogModal({
  ...
  const onPickFile = (f: File | null | undefined) => {
    ...
    if (!looksLikeNetworkLog(f.name)) {
      setErr(
        `"${f.name}" doesn't look like a network log (.csv / .json / .xlsx / .txt). It will still be sent — the backend will reject it if the format isn't recognised.`,
      );
    }
    setStaged(f);
  };
  ...
  return (
    <Modal title="Ingest network log" onClose={onClose} width={560}>
      ...
      >
        ...
        → Parse · Analyze · Decide · Persist · Project. Re-uploading the same
        bytes returns the prior run (deduplicated).
      </div>
      ...
      {tab === "url" && (
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <div className="mono" style={{ fontSize: 10, color: "var(--ink-3)" }}>
            Tip: for a OneDrive personal share link, append{" "}
            <code>?download=1</code> (or
            <code> &amp;download=1</code>) to force a direct file response. The
            browser fetches the bytes; the backend never sees the URL. If the
            fetch is blocked by CORS, download the file locally and use the
            &ldquo;Upload file&rdquo; tab.
          </div>
        </div>
      )}
      ...
      {busy && (
        <div
          className="mono"
          style={{ fontSize: 11, color: "var(--accent)", marginTop: 12 }}
        >
          ◐ {stage || "Working…"}
        </div>
      )}
      ...
      {err && (
        <div
          className="mono"
          style={{ fontSize: 11, color: "var(--crit)", marginTop: 12 }}
        >
          ⚠ {err}
        </div>
      )}
    ...
    try {
      await Promise.all(
        Array.from(files).map(async (file) => {
          ...
          fd.append(
            "title",
            files.length > 1 ? file.name : form.title || file.name,
          );
  ...
  return (
    <Modal title="Upload document" onClose={onClose}>
      <div
        ...
        style={{ fontSize: 10, color: "var(--ink-3)", marginBottom: 10 }}
      >
        For SOPs / runbooks / incident reports. Indexed into the RAG corpus the
        Copilot retrieves against. For network logs, use{" "}
        <strong style={{ color: "var(--ink-2)" }}>+ Ingest log</strong> instead.
      </div>
      <form onSubmit={submit} style={{ display: "grid", gap: 10 }}>
        ...
        <Field label="Title (optional)">
          <input
            ...
            placeholder={
              selectedCount > 1 ? "Auto-generated from filenames" : ""
            }
          />
        </Field>
...
function LinkModal({
  ...
  return (
    <Modal title="Link cloud document" onClose={onClose}>
      <form onSubmit={submit} style={{ display: "grid", gap: 10 }}>
        <div className="mono" style={{ fontSize: 10, color: "var(--ink-3)" }}>
          Linking registers the document in the index. Ingestion calls the
          provider on demand &mdash; providers shown as &ldquo;not
          connected&rdquo; will fail until an SDK adapter is wired up.
        </div>
        <Field label="Source">
          <select
            style={inputStyle}
        ...
        <Field label="Storage key (Drive file ID / blob name / SharePoint ID)">
          <input
            style={inputStyle}
            required
    ...
    <Modal title="Confirm Deletion" onClose={onClose}>
      <div style={{ display: "grid", gap: 16 }}>
        <div style={{ fontSize: 13, color: "var(--ink)" }}>
          ...
          <b style={{ color: "var(--crit)" }}>{doc.title}</b>?
        </div>
        <div
          className="mono"
          style={{
        ...
        >
          <Btn type="button" onClick={onClose} disabled={busy}>
            Cancel
          </Btn>
          <Btn
            type="button"
