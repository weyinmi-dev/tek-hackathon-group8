...
function Login({ onAuth }){
  const [email, setEmail] = React.useState('oluwaseun.a@telco.lag');
  ...
  const submit = (e) => {
    ...
    setBusy(true);
    setTimeout(()=>{ onAuth({ method:'password' }); }, 700);
  };
  const sso = () => {
    setBusy(true);
    setTimeout(()=>{ onAuth({ method:'azure-ad' }); }, 600);
  };
    ...
    <div style={{minHeight:'100vh',display:'grid',gridTemplateColumns:'1fr 1fr',background:'var(--bg)'}}>
      ...
      <div style={{
        ...
        <div style={{position:'relative',display:'flex',gap:24,fontFamily:'var(--mono)',fontSize:10.5,color:'var(--ink-3)',letterSpacing:'.10em'}}>
          ...
          <span><span style={{color:'var(--ok)'}}>●</span> AZURE AD</span>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'center',padding:42}}>
        <form onSubmit={submit} style={{width:380,display:'flex',flexDirection:'column',gap:18}}>
          ...
          <button type="button" onClick={sso} style={{
          ...
          <Field label="Password" right={<a style={{fontSize:11,color:'var(--ink-3)'}} href="#">Forgot?</a>}><input type="password" value={pw} onChange={e=>setPw(e.target.value)} style={inp}/></Field>
          ...
          <button type="submit" disabled={busy} style={{
            appearance:'none',padding:'12px',cursor:busy?'wait':'pointer',
            background:'var(--accent)',color:'#001a10',
          ...
          <div className="mono" style={{fontSize:10,color:'var(--ink-3)',textAlign:'center',marginTop:6,letterSpacing:'.04em'}}>
            ALL SIGN-IN ATTEMPTS LOGGED · TLS 1.3 · OAuth2
          </div>
        </form>
      </div>
    </div>
