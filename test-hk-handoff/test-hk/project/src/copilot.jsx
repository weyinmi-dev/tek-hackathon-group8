...
const SUGGESTED = [
  ...
  "Why did Surulere consume more diesel yesterday?",
  "Which sites are at risk of fuel theft?",
  "Recommend cost optimizations for Lekki cluster",
...
function Copilot({ embedded, role }){
  ...
  const ask = async (q) => {
    ...
    let answer = '';
    try {
      const sys = `You are TelcoPilot, an AI assistant embedded in a telco Network Operations Center for a Lagos, Nigeria metro carrier. You answer questions about both NETWORK performance (signal, packet loss, outages, towers) AND ENERGY/POWER management (grid, diesel, battery, solar, fuel theft, OPEX).

Live state (JSON):
${JSON.stringify({towers:TOWERS.slice(0,6), alerts:ALERTS.slice(0,3), sites:(window.SITES||[]).slice(0,8), anomalies:(window.ANOMALY_EVENTS||[]).slice(0,4)})}
