...
public static class AlertsSeeder
{
    ...
    {
        // Heal v1 seed rows where the tower code was concatenated with an "and N more"
        // suffix ("TWR-LAG-W-014 +2"). That field is the OSM geo-lookup key (Tower.Code),
        // so the suffix broke geo enrichment for INC-2840 — `geo` was always null and the
        // OSM GEO CONTEXT panel never rendered. Runs before the early-return so existing
        // dev DBs get patched without a wipe; idempotent on already-correct rows.
        await db.Alerts
            .Where(a => a.TowerCode == "TWR-LAG-W-014 +2")
            .ExecuteUpdateAsync(s => s.SetProperty(a => a.TowerCode, "TWR-LAG-W-014"), ct);
        ...
        var seeds = new (string code, AlertSeverity sev, string title, string region, string tower, string cause, int users, double conf, AlertStatus status, int minutesAgo)[]
        {
            ...
            ("INC-2840", AlertSeverity.Critical, "3 towers offline — power cluster",       "Lagos West", "TWR-LAG-W-014",   "Grid failure — IKEDC sector 7",                                                    38400, 0.88, AlertStatus.Active,        8),
